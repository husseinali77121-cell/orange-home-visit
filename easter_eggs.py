...
#  مفتاح التشغيل العام
# ════════════════════════════════════════════════════════════════
EGG_VERSION = "1.8"      # للتأكد إن النسخة الجديدة هي اللي شغالة فعلاً
...
def client_comment(name: str = "", phone: str = "", tag: str = "",
    ...
    if age_years is not None:
        if not age_years:
            ...
            out.append(_pick(f"baby{age_years}", [
...
#       الرسالة بتتغيّر كل يوم، وثابتة طول اليوم عشان ماتلخبطش.
#       عايز تضيف تحية للاسيتيه أو الأدمن؟ ضيف المفتاح هنا وبس.
# ════════════════════════════════════════════════════════════════
GREETING_SECONDS = 10    # مدة ظهور بانر التحية قبل ما يختفي لوحده
...
def user_greeting(user_type: str, name: str | None = None) -> str | None:
    """تحية عند الدخول حسب نوع المستخدم والوقت. بتتغيّر كل يوم."""
    if not FUN_MODE:
        return None
    ...
    if not opts:
        return None
    # البذرة = المستخدم + التاريخ + وقت اليوم → رسالة ثابتة طول الفترة
    msg = _pick(f"{user_type}|{now.date()}|{part}", opts)
