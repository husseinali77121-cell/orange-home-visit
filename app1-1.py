...
# على Streamlit Cloud: /data متاح ودائم — fallback لنفس المجلد
...
def make_whatsapp_msg(v, target="internal"):
    ...
    if target == "client":
        ...
        addr_d = v.get("address","") or ""
        # بناء سطر العنوان: المدينة / الحي \n العنوان
        location_parts = []
        if city_d: location_parts.append(city_d)
        if dist_d: location_parts.append(dist_d)
        location_header = " / ".join(location_parts) if location_parts else ""
        addr_line = f"📍 *عنوان الزيارة:* {location_header}\n{addr_d}\n" if location_header else f"📍 *عنوان الزيارة:* {addr_d}\n"
        loc_line_g = f"🗺️ {v.get('location_link','')}\n" if v.get("location_link") else ""
        return (f"🟠 *زيارة منزلية*\n━━━━━━━━━━━━━━\n"
                f"👨‍⚕️ *الدكتور القائم بالزيارة:* {doc}\n📅 *الموعد:* {dt_str}\n"
                f"━━━━━━━━━━━━━━\n{addr_line}{loc_line_g}")
    else:
