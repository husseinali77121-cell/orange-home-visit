...
def archive_and_prune(cutoff_iso, user_email=""):
    ...
    conn.execute("UPDATE visits SET archived=1 WHERE COALESCE(archived,0)=0 AND visit_date<=?",
                 (cutoff_iso,))
    conn.commit()
    ...
    if not ok:
        conn.execute("UPDATE visits SET archived=0 WHERE archived=1 AND visit_date<=?", (cutoff_iso,))
        conn.commit()
...
def _boot_pull_from_github():
    ...
    ok = load_from_github_json(retries=4, backoff=2, upsert=True)
    return {"ok": ok, "err": _GH_STATE.get("last_error", ""),
            "rows": _GH_STATE.get("boot_pulled", 0), "at": datetime.now().isoformat()}
