...
def load_from_github_json():
    """تحميل البيانات من GitHub عند أول تشغيل (صامت — بيستخدم _gh_download_visits)."""
    ...
    for rec in visits:
        rec.pop("_user", None)
        try:
            loaded += _load_insert_visit(conn, rec)
        except Exception:
            pass
    conn.commit()
    return loaded > 0
...
# ── تحميل البيانات من GitHub عند أول تشغيل (لو الـ DB فاضية) ──
if "github_loaded" not in st.session_state:
    st.session_state.github_loaded = True
    conn_check = get_connection()
    count_check = conn_check.execute("SELECT COUNT(*) FROM visits WHERE deleted_at IS NULL").fetchone()[0]
    if count_check == 0 and GITHUB_TOKEN and GITHUB_REPO:
        load_from_github_json()
