...
def save_to_github_json():
    ...
    try:
        ...
        sha = _get_github_file_sha(GITHUB_JSON_PATH)
        body = {
            "message": f"auto-save {datetime.now().strftime('%Y-%m-%d %H:%M')} ({local_total} visits)",
            "content": content_b64,
        ...
        req = urllib.request.Request(url, data=req_body, headers={**_github_headers(),"Content-Type":"application/json"}, method="PUT")
        try:
            with urllib.request.urlopen(req, timeout=30) as r:
                ok = r.status in [200, 201]
        except urllib.error.HTTPError as he:
            _GH_STATE["connected"]  = False
            _GH_STATE["last_error"] = f"فشل الحفظ على GitHub — HTTP {he.code}" + (" (توكن منتهي/صلاحيات ناقصة)" if he.code in (401, 403) else "")
            return False
        if ok:
...
def load_from_github_json(retries=3, backoff=2):
    """
    تحميل صامت من GitHub مع إعادة محاولة (مهم وقت الـ cold-start).
    قراءة فقط: بيقرأ من GitHub ويدرج محليًا (INSERT OR IGNORE) — عمره ما يكتب/يمسح على GitHub.
...
# ── تحميل من GitHub — يفضل يحاول طول ما الـ DB فاضية (مش مرة واحدة) ──
# ملاحظة أمان: ده مسار قراءة فقط، مفيش أي كتابة/حذف على GitHub هنا إطلاقًا.
if GITHUB_TOKEN and GITHUB_REPO and not st.session_state.get("github_loaded", False):
    conn_check = get_connection()
    count_check = conn_check.execute(
        "SELECT COUNT(*) FROM visits WHERE deleted_at IS NULL").fetchone()[0]
    if count_check > 0:
        st.session_state.github_loaded = True          # فيه بيانات أصلًا → خلاص
    elif load_from_github_json():
        st.session_state.github_loaded = True          # اتعلّم بس بعد نجاح الاتصال
    # لو فشل التحميل → الـ flag مش هيتعلّم → هيحاول تلقائيًا في أول rerun بدون أي زر
...
def _load_insert_visit(conn, rec):
    """
    ...
    cur = conn.execute("""
        INSERT OR IGNORE INTO visits (
            ...
            payment_status,payment_method,paid_amount,payment_date,deleted_at
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
...
if st.session_state.page == "home":
    ...
    with st.expander("💾 نسخ احتياطي واسترجاع", expanded=False):
        ...
        if st.session_state.user_type == "admin":
            st.markdown("---")
            st.markdown("**☁️ استرجاع مباشر من GitHub**")
            gh_all = st.checkbox("استورد كل الزيارات كنشطة (تجاهل المحذوف)", key="gh_all_active")
