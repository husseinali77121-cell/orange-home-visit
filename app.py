...
def save_to_github_json():
    """
    حفظ كل الزيارات على GitHub كـ Visits.json — يُستدعى بعد كل تعديل.
    محمي بحُرّاس ضد الكتابة الكارثية:
      1) عمره ما يكتب لو الـ DB المحلي فاضي.
      2) عمره ما يكتب عدد أقل من الموجود على GitHub
         (الحذف soft delete → العدد الكلي لا ينقص أبدًا في التشغيل السليم).
         الاستثناء الوحيد: allow_shrink_once=True اللي بتتحط من archive_and_prune() بس.
      3) لو مش قادر يتحقق من GitHub أصلًا → يرفض بأمان.
    """
    # أي استدعاء هنا = حصلت كتابة محلية → اقفل upsert لباقي عمر الكونتينر
    ...
    try:
        ...
        if remote_total is None:
            _gh_download_visits()               # بيحدّث _GH_STATE داخليًا
            remote_total = _GH_STATE.get("remote_total")
        if remote_total is None:
            _GH_STATE["last_save_blocked"] = "اترفض الحفظ: تعذّر التحقق من GitHub — " + (_GH_STATE.get("last_error") or "اتصال فاشل")
            _GH_STATE["unsaved"] = _GH_STATE.get("unsaved", 0) + 1
            return False
        if local_total < remote_total and not _GH_STATE.get("allow_shrink_once"):
...
def archive_and_prune(cutoff_iso, user_email=""):
    ...
    conn.executemany("UPDATE visits SET archived=1 WHERE id=?", [(i,) for i in touched])
    conn.commit()
    ...
    if not ok:
        ...
        conn.executemany("UPDATE visits SET archived=0 WHERE id=?", [(i,) for i in touched])
        conn.commit()
...
def _load_insert_visit(conn, rec, upsert=False, archived=0):
    ...
    cur = conn.execute(f"""
        {_verb} INTO visits (
            ...
            payment_status,payment_method,paid_amount,payment_date,deleted_at,archived
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (
