...
def _get_github_file_sha(path):
    """جلب SHA الحالي للملف على GitHub"""
...
def _gh_download_visits():
    """
    يحمّل Visits.json من GitHub بأمان.
    ...
    """
    if not GITHUB_TOKEN:
        return None, None, "github_token مش موجود في Secrets"
    if not GITHUB_REPO:
        return None, None, "github_repo مش موجود في Secrets"
    try:
        ...
        try:
            ...
            code = he.code
            if code == 401:
                return None, code, "401 غير مصرح — التوكن غلط أو منتهي الصلاحية"
            if code == 403:
                return None, code, "403 ممنوع — صلاحيات التوكن ناقصة (محتاج Contents) أو تجاوز rate limit"
            if code == 404:
                return None, code, "404 مش موجود — راجع github_repo والفرع (main) واسم الملف Visits.json"
            body = ""
            try:
                body = he.read().decode("utf-8", "ignore")[:200]
            except Exception:
                pass
            return None, code, f"HTTP {code}: {body}"
