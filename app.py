...
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS audit_log (
            ...
            record_id TEXT, details TEXT, timestamp TEXT
        )
    """)
    NEW_COLS = [
        ("age_unit","TEXT DEFAULT 'سنة'"), ("status","TEXT DEFAULT 'مجدولة'"),
    ...
    for col, defn in NEW_COLS:
        try:
            conn.execute(f"ALTER TABLE visits ADD COLUMN {col} {defn}")
        except sqlite3.OperationalError:
            pass
    conn.commit()
    _ensure_default_doctors(conn)
    # ── تثبيت حالة الدفع للزيارات القديمة (تشغيل مرة واحدة) ──
    conn.execute("""
        UPDATE visits
        ...
        WHERE deleted_at IS NULL
          AND (payment_status IS NULL OR payment_status = '' OR payment_status = 'غير مدفوع')
    """)
    conn.commit()
...
def _log_audit(user_email, action, table_name, record_id, details=""):
    try:
        conn = get_connection()
        conn.execute(
            "INSERT INTO audit_log (user_email,action,table_name,record_id,details,timestamp) VALUES (?,?,?,?,?,?)",
            (user_email or "", action, table_name, record_id, details, datetime.now().isoformat())
        )
        conn.commit()
    except Exception:
        pass
...
def insert_visit(record):
    ...
    _log_audit(record.get("_user",""), "insert", "visits", record["id"])
...
def update_payment(vid, pay_status, pay_method, paid_amount, pay_date, user_email=""):
    ...
    conn.commit()
    _log_audit(user_email, "update_payment", "visits", vid, f"{pay_status}|{paid_amount}ج")
...
def get_smart_suggestions(selected_labs_list, price_lookup, phone=None):
    ...
    if phone:
        try:
            history = fetch_client_history(phone)
            historical_labs = set()
            for v in history[:5]:
...
if st.session_state.page == "home":
    ...
    all_vs  = fetch_visits({"branch":bf_kpi} if bf_kpi else {})
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not today_visits:
        ...
        with tc2:
            ...
            tomorrow_visits = fetch_visits({**({k:v for k,v in f.items() if k!="date_exact"}), "date_exact": tomorrow})
            if tomorrow_visits:
        ...
        if prev:
            ...
            for old_v in prev[:4]:
                ...
                st.markdown(f'<div class="history-card">📅 {old_date} — <span style="background:{old_sc};color:#fff;border-radius:6px;padding:1px 8px;font-size:11px;">{old_stat}</span>'
                            ...
                            f'<br>💰 {old_v.get("total_price",0):,} جنيه — 🧪 {len((old_v.get("selected_labs_text","") or "").splitlines())} تحليل</div>',
                            unsafe_allow_html=True)
        ...
        if not all_client_visits:
            ...
            for v in all_client_visits:
                ...
                st.markdown(f'<div class="history-card">'
                            ...
                            f'<br>💰 {v.get("total_price",0):,} جنيه — 🧪 {len((v.get("selected_labs_text","") or "").splitlines())} تحليل</div>', unsafe_allow_html=True)
