...
SCHEMA_VERSION = 2  # بدءاً من الإصدار 2 بعد التعديلات
...
def init_db():
    ...
    if current is None or current < SCHEMA_VERSION:
        ...
        # إذا كان الإصدار الحالي 0 (أول تشغيل) أو أقل من 2، نضيف الأعمدة المفقودة
        # نستخدم دالة add_column_if_missing لتجنب الأخطاء
        def add_column_if_missing(table, column, definition):
            try:
                conn.execute(f"ALTER TABLE {table} ADD COLUMN {column} {definition}")
            except sqlite3.OperationalError:
                pass  # العمود موجود بالفعل
        # الأعمدة التي قد تكون مفقودة في الإصدارات القديمة
        columns_to_add = [
    ...
    # ── التأكد من وجود الأطباء الافتراضيين ──
...
def log_error(user_email, page, exception, stack_trace=""):
    ...
    try:
        ...
        pass  # إذا فشل تسجيل الخطأ، تجاهل
...
def restore_database(backup_file):
    ...
    try:
        ...
        # إعادة تهيئة الاتصال (سيتم تحديثه تلقائياً)
        return True
    except Exception as e:
...
# ══════════════════════════════════════════════════════════════════════════════
# دالة البحث الذكي (جديدة)
# ══════════════════════════════════════════════════════════════════════════════
def fetch_visit_by_unique_keys(name, phone, visit_date):
    """البحث عن زيارة بناءً على الاسم والتليفون والتاريخ بدلاً من الـ ID"""
    conn = get_connection()
    row = conn.execute(
...
def fetch_visits(filters=None, page=None, page_size=None):
    """جلب الزيارات مع دعم pagination"""
    conn = get_connection()
...
def update_status_only(vid, new_status):
    ...
    conn.commit()
    if old:
        _log_audit(st.session_state.user_email, "update_status", "visits", vid,
                   field_name="status", old_value=old.get("status"), new_value=new_status)
...
def update_rating(vid, rating):
    ...
    conn.commit()
    if old:
        _log_audit(st.session_state.user_email, "update_rating", "visits", vid,
                   field_name="rating", old_value=old.get("rating"), new_value=rating)
...
def update_tag(vid, tag):
    ...
    conn.commit()
    if old:
        _log_audit(st.session_state.user_email, "update_tag", "visits", vid,
                   field_name="tag", old_value=old.get("tag"), new_value=tag)
...
def update_payment(vid, pay_status, pay_method, paid_amount, pay_date, user_email=""):
    ...
    conn.commit()
    if old:
        _log_audit(user_email, "update_payment", "visits", vid,
                   field_name="payment_status", old_value=old.get("payment_status"), new_value=pay_status)
...
# ══════════════════════════════════════════════════════════════════════════════
# Excel Export/Import (مع استخدام format_money)
# ══════════════════════════════════════════════════════════════════════════════
def export_to_excel(branch_filter=None, month=None, year=None, date_from=None, date_to=None):
    ...
    visits, _ = fetch_visits(filters if filters else None, page=None, page_size=None)  # جلب الكل للتصدير
...
def import_from_excel(uploaded_file):
    try:
        ...
        # خريطة مرنة لتحديد أسماء الأعمدة (عربي / إنجليزي)
        col_mapping = {}
        arabic_to_internal = {
            "id": ["رقمالزيارة", "id", "visitid", "معرفالزيارة", "الزيارةرقم"],
            ...
            "age": ["السن", "العمر", "age"],
            "age_unit": ["وحدةالعمر", "العمربوحدة", "age_unit"],
            "doctor_name": ["الدكتور", "اسمالدكتور", "طبيب", "doctor"],
            "branch": ["الفرع", "branch"],
            "address": ["العنوان", "address"],
            "location_link": ["رابطالموقع", "location", "map", "رابط"],
            "selected_labs_text": ["التحاليل", "التحليل", "labs", "tests", "selected_labs_text"],
            "notes": ["ملاحظات", "notes"],
            "labs_price_before": ["السعرقبلالخصم", "قبلالخصم", "pricebefore", "labs_price_before"],
            "labs_price_after": ["السعربعدالخصم", "بعدالخصم", "priceafter", "labs_price_after"],
            "transport_fee": ["بدلالانتقال", "النقل", "transportfee", "transport_fee"],
        ...
        # بناء خريطة الأعمدة بناءً على الملف المرفوع
        for internal_key, possible_names in arabic_to_internal.items():
        ...
        for _, row in df.iterrows():
            ...
            for num_key in ["labs_price_before", "labs_price_after", "transport_fee", "total_price", "paid_amount", "age"]:
                if num_key in record and record[num_key] is not None:
                    try: record[num_key] = float(record[num_key])
                    except: record[num_key] = 0
