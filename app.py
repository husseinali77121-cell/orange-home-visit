...
def export_to_excel(branch_filter=None, month=None, year=None, date_from=None, date_to=None):
    ...
    elif branch_filter == "La Cite":
        fname = f"lacite_{period}.xlsx"
    else:
    ...
    c = ws.cell(1, 1, f"🟠 Orange Lab Home Visit Management System — {branch_label} — {month_label}")
    ...
    # حساب كل دكتور
    # الأطباء الذين يحق لهم بدل الانتقال في الجدول التلخيصي
    TRANSPORT_ELIGIBLE = ["محمد شفيق"]
    ...
    for _, row in df_valid.iterrows():
        ...
        # بدل الانتقال في الجدول التلخيصي لـ محمد شفيق فقط
...
<body>
  <div class="header">
    <h2>🟠 Orange Lab Home Visit Management System</h2>
...
if st.session_state.page == "home":
    ...
    with cf1:
        if st.session_state.user_type == "diamond":
            ...
            selected_branch = "La Cite"
            st.selectbox("الفرع", ["La Cite"], disabled=True)
        else:
    ...
    all_vs  = fetch_visits({"branch":"Diamond"} if st.session_state.user_type=="diamond" else {"branch":"La Cite"} if st.session_state.user_type=="lacite" else {})
    ...
    if st.session_state.user_type in ["admin","diamond","lacite"]:
        with st.expander("📤 تصدير إلى Excel", expanded=False):
            ...
            if st.session_state.user_type == "admin":
                ...
                bf_exp    = "La Cite"
                btn_label = "📤 تصدير زيارات La Cite"
            else:
                bf_exp    = "Diamond"
            ...
            if st.button(btn_label, use_container_width=True, key="btn_export_home"):
                if not exp_from or not exp_to:
                    ...
                    if df_ex.empty:
                        ...
                        fname_dl = f"{'diamond' if bf_exp=='Diamond' else 'lacite' if bf_exp=='La Cite' else 'visits'}_{exp_from}_{exp_to}.xlsx"
    ...
    <div style="text-align:center;margin-top:50px;padding-top:20px;border-top:2px solid #FF6B00;
      ...
      Orange Lab Home Visit Management System<br>Developed by <b>Dr / Hussein Ali</b> 2026 For <span style="color:#FF6B00;">Orange Lab 🍊</span>
    </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with rb:
        if st.session_state.user_type=="diamond":
            ...
            branch_filter="La Cite"; st.selectbox("الفرع",["La Cite"],disabled=True)
        else:
    ...
    with st.expander("📤 تصدير إلى Excel", expanded=False):
        ...
        if st.session_state.user_type == "admin":
            ...
            bf_rep = "La Cite"
            btn_rep_label = "📤 تصدير زيارات La Cite"
        else:
            bf_rep = "Diamond"
        ...
        if st.button(btn_rep_label, use_container_width=True, key="btn_export_reports"):
            if not rep_from or not rep_to:
                ...
                if df_rep.empty:
                    ...
                    fname_rep   = f"{'diamond' if bf_rep=='Diamond' else 'lacite' if bf_rep=='La Cite' else 'visits'}_{rep_from}_{rep_to}.xlsx"
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for v in all_vs:
        ...
        try:
            ...
            if mk not in monthly:
                monthly[mk] = {"rev":0,"count":0,"year":d.year,"month":d.month}
            monthly[mk]["rev"]   += v.get("total_price",0)
            monthly[mk]["count"] += 1
        except: pass

    sorted_m  = sorted(monthly.keys(), key=lambda k:(monthly[k]["year"],monthly[k]["month"]))[-6:]
    months_labels = sorted_m
    months_rev    = [monthly[m]["rev"]   for m in sorted_m]
    months_count  = [monthly[m]["count"] for m in sorted_m]
    ...
    status_counts = {}
    for v in all_vs:
        s = v.get("status","مجدول")
        status_counts[s] = status_counts.get(s,0)+1
    canc_rate = round(status_counts.get("ملغية",0)/total_all*100,1) if total_all else 0
    ...
    st.markdown(f"""    <div class="stat-grid">
      ...
      <div class="stat-box"><div class="stat-num">{today_cnt}</div><div class="stat-label">زيارات اليوم</div></div>
      <div class="stat-box"><div class="stat-num" style="font-size:13px">{rev_all:,.0f}</div><div class="stat-label">إجمالي الإيراد</div></div>
      <div class="stat-box"><div class="stat-num" style="font-size:13px">{week_rev:,.0f}</div><div class="stat-label">إيراد آخر 7 أيام</div></div>
    </div>
    <div class="stat-grid">
      <div class="stat-box"><div class="stat-num" style="color:#E74C3C">{canc_rate}%</div><div class="stat-label">نسبة الإلغاء</div></div>
      <div class="stat-box"><div class="stat-num" style="font-size:13px;color:#9B59B6">{top_doc}</div><div class="stat-label">أكثر طبيب نشاطاً</div></div>
      <div class="stat-box"><div class="stat-num" style="font-size:12px;color:#3498DB">{top_area}</div><div class="stat-label">أكثر منطقة</div></div>
    ...
    if HAS_PLOTLY:
        # ── Row 1: عدد الزيارات + الإيراد الشهري ──
        ch1, ch2 = st.columns(2)
        with ch1:
            st.markdown("##### 📅 عدد الزيارات شهرياً")
            fig = go_plt.Figure(go_plt.Bar(
                x=months_labels, y=months_count,
                marker_color="#FF6B00", text=months_count, textposition="outside"
            ))
            fig.update_layout(height=280, margin=dict(t=20,b=20,l=10,r=10),
                              plot_bgcolor="#FFF8F0", paper_bgcolor="#FFF8F0",
                              font=dict(family="Cairo"), yaxis=dict(showgrid=True,gridcolor="#FFE8D1"))
            st.plotly_chart(fig, use_container_width=True)

        with ch2:
            st.markdown("##### 💰 الإيراد الشهري (جنيه)")
            fig2 = go_plt.Figure(go_plt.Bar(
                x=months_labels, y=months_rev,
                marker_color="#27AE60",
            ...
            fig2.update_layout(height=280, margin=dict(t=20,b=20,l=10,r=10),
                               ...
                               font=dict(family="Cairo"), yaxis=dict(showgrid=True,gridcolor="#D5F5E3"))
            st.plotly_chart(fig2, use_container_width=True)

        st.markdown("---")

        # ── Row 2: نسبة الإلغاء (Pie) + أفضل طبيب (Bar) ──
        ch3, ch4 = st.columns(2)
        with ch3:
            st.markdown("##### 🔖 توزيع الحالات")
            STATUS_PIE_COLORS = {"تمت":"#27AE60","مجدولة":"#3498DB",
                                  "في الطريق":"#F39C12","ملغية":"#E74C3C"}
            labels = list(status_counts.keys())
            values = list(status_counts.values())
            colors = [STATUS_PIE_COLORS.get(l,"#888") for l in labels]
            fig3 = go_plt.Figure(go_plt.Pie(
        ...
        with ch4:
            ...
            docs_sorted = sorted(doc_stats.items(), key=lambda x:x[1]["count"], reverse=True)[:6]
            doc_names  = [d[0] for d in docs_sorted]
            doc_counts_list = [d[1]["count"] for d in docs_sorted]
            fig4 = go_plt.Figure(go_plt.Bar(
                y=doc_names, x=doc_counts_list, orientation="h",
                marker_color="#9B59B6",
                text=doc_counts_list, textposition="outside"
            ))
        ...
        # ── Row 3: أكثر التحاليل + أفضل المناطق ──
        ...
        if branch_data:
            ...
            fig7 = go_plt.Figure()
            fig7.add_trace(go_plt.Bar(name="الزيارات", x=df_br["الفرع"], y=df_br["الزيارات"], marker_color="#FF6B00"))
            fig7.add_trace(go_plt.Bar(name="تمت",       x=df_br["الفرع"], y=df_br["تمت"],       marker_color="#27AE60"))
            fig7.update_layout(height=250, barmode="group", font=dict(family="Cairo"),
                               margin=dict(t=10,b=10,l=10,r=10))
            st.plotly_chart(fig7, use_container_width=True)

    else:
        # Fallback بدون Plotly
