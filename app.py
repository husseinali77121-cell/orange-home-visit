...
def make_whatsapp_msg(v, target="internal"):
    ...
    if target == "client":
        ...
        city_d = v.get("city","") or ""
        dist_d = v.get("district","") or ""
        location_parts = []
        if city_d: location_parts.append(city_d)
        if dist_d: location_parts.append(dist_d)
        location_header = " / ".join(location_parts) if location_parts else addr
