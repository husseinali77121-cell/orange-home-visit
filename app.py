...
# ── 🍊 رسائل الإنجازات المؤجلة (اتخزنت قبل الانتقال لصفحة التفاصيل) ──
for _egg_m in st.session_state.pop("_egg_toasts", []):
    egg.render_toast(st, _egg_m, "🧿")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not is_edit:
        ...
        if _egg_chk:
            if _egg_chk.get("level") == "blocker":
                ...
                _egg_ans = st.radio(
                    "اختار واحدة:", _egg_chk["choices"],
                    index=None, key="egg_past_choice",
                )
    ...
    if st.button("💾 حفظ الزيارة" if not is_edit else "💾 حفظ التعديلات", use_container_width=True):
        if not egg_allow_save:
            ...
            # ── 🍊 إنجازات اليوم (فرع / دكتور) — بتتعرض كـ toast بعد الانتقال ──
            if not is_edit:
                try:
                    _egg_conn = get_connection()
                    _egg_bc = _egg_conn.execute(
                        "SELECT COUNT(*) FROM visits WHERE visit_date=? AND branch=? "
                        "AND status!='ملغية' AND deleted_at IS NULL",
                        (visit_date.isoformat(), branch)
                    ).fetchone()[0]
                    _egg_dc = get_doctor_workload(doctor_name, visit_date.isoformat())
                    st.session_state["_egg_toasts"] = [m for m in (
                        egg.daily_milestone(_egg_bc, "branch", branch),
                        egg.daily_milestone(_egg_dc, "doctor", doctor_name),
                    ) if m]
                except Exception:
                    pass
