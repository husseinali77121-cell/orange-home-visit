...
def save_to_github_json():
    ...
    try:
        ...
        if remote_total is None:
            _gh_load_all_visits() if USE_MONTHLY_SYNC else _gh_download_visits()
            remote_total = _GH_STATE.get("remote_total")
        if remote_total is None:
...
def _auto_backup_json():
    """نسخ احتياطي تلقائي JSON بعد كل عملية حفظ — صامت"""
    try:
        conn = get_connection()
        rows = conn.execute("SELECT * FROM visits WHERE deleted_at IS NULL").fetchall()
        data = [dict(r) for r in rows]
        os.makedirs(os.path.dirname(BACKUP_JSON) if os.path.dirname(BACKUP_JSON) else ".", exist_ok=True)
        with open(BACKUP_JSON, "w", encoding="utf-8") as f:
            json.dump({"exported_at": datetime.now().isoformat(), "visits": data}, f, ensure_ascii=False, indent=2)
    except Exception:
        pass
    ...
    if st.button("💾 حفظ الزيارة" if not is_edit else "💾 حفظ التعديلات", use_container_width=True):
        if not name or not phone or not address:
            ...
            record = {
                ...
                "branch": branch,
                "city": city if 'city' in dir() else pf.get("city",""),
                "district": district if 'district' in dir() else pf.get("district",""),
