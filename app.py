...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── 🍊 التاريخ الحي (من الـ widget لو اتفتح، وإلا prefill/النهاردة) ──
    _egg_vdate = st.session_state.get("visit_date_inp") or visit_date_temp
    # ── 🍊 تعليق على عدد زيارات الدكتور في نفس اليوم ──
    try:
        _egg_docn = get_doctor_workload(doctor_name, _egg_vdate.isoformat()) + (0 if is_edit else 1)
        egg.render_caption(st, egg.daily_milestone(_egg_docn, "doctor", doctor_name))
    except Exception:
        pass

    branch    = st.selectbox("🏥 الفرع", ["La Cite","Diamond"], index=0 if pf.get("branch","La Cite")=="La Cite" else 1)
    # ── 🍊 تعليق على عدد زيارات الفرع في نفس اليوم ──
    try:
        _egg_bn = get_connection().execute(
            ...
            (_egg_vdate.isoformat(), branch)
        ).fetchone()[0] + (0 if is_edit else 1)
        egg.render_caption(st, egg.daily_milestone(_egg_bn, "branch", branch))
    except Exception:
        pass
