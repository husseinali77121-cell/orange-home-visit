...
def price_comment(total: float, discount_pct: float = 0.0) -> str | None:
    ...
    if discount_pct < 0:
        return _pick(str(total), [
            "🕵️ السعر بعد الخصم أعلى من قبل الخصم! لو الرقم ده صح يبقى انت حرامي 😂",
            "😂 خصم بالسالب؟ لو الرقم ده مظبوط يبقى انت حرامي — راجعه",
            "🚨 بعد الخصم أغلى من قبله! يا إما غلطة يا إما انت حرامي 😂",
        ])
    if discount_pct >= 50:
