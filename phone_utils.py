...
def wa_digits(phone):
    ...
    ★ اتصلّح: الأرقام المكتوبة بأرقام عربية (٠١٠…) كانت بتعدّي من غير تحويل
      وتطلع لينك مكسور زي "20٠١٠١٦٨٧٢٨٠١". clean_digits بتحوّلها وتشيل
      أي مسافات/شرط/أقواس قبل أي حاجة تانية.
    """
    p = str(phone or "").strip()
    is_intl = p.startswith("+")
    p = clean_digits(p)              # ← أرقام إنجليزية نضيفة بس
    if not p:
        return ""
    if is_intl:
        return p                     # مفيش لزوم نضيف 20
    if p.startswith("0"):
        return "20" + p[1:]
...
def render_phone_input(st, label, value="", key_prefix="ph", placeholder="1xxxxxxxxx",
    ...
    # ⚠️ لستة محلية — ممنوع نعدّل CODES العامة من جوه دالة عرض، لأنها بتفضل
    #    متغيّرة لكل المستخدمين في نفس الحاوية.
    opts = list(CODES) if code0 in CODES else [code0] + list(CODES)
    c1, c2 = st.columns([1, 2.4])
    with c1:
        code = st.selectbox(
            "المفتاح", opts, index=opts.index(code0),
