...
def _boot_pull_from_github():
    ...
    # upsert آمن **بس** طول ما محصلش أي كتابة محلية في الكونتينر ده.
    # لو السحب فشل وأعدنا المحاولة بعد ما المستخدم عدّل → نرجع لـ INSERT OR IGNORE
    # عشان GitHub ميدوسش على تعديل لسه في الـ DB المحلي.
    safe_upsert = (_GH_STATE.get("local_writes", 0) == 0)
